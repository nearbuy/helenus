  ...
  for(; i < len; i += 1){
    ...
    if(item.name !== 'KEY'){
      ...
      if(schema.noDeserialize) {
        ...
        try {
          var val = deserializeValue(item.value);
        } catch(e) {
          throw new Error("Error deserializing value for column " + name + ": " + e.message);
        }
        this.push(new Column(name, val, item.timestamp, item.ttl));
      }
